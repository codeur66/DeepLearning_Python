...
class Train:
    ...
    def __init__(self, model_architecture, model_name):
        self.model_archit = model_architecture
    ...
    def set_hyperparameters(self):
        self.model_archit.model.compile(optimizer=self.optimizer,
                                        loss=self.loss,
    ...
    def trainning(self):
        self.model_archit.model.fit(x=self.train_images,
                                    y=self.train_labels,
    ...
    def evaluate(self):
        test_loss, test_acc = self.model_archit.model.evaluate(self.test_images,
                                                               self.test_labels)
    ...
    def save_model_and_get_path(self):
        self.model_archit.model.save()
        model_path = getcwd() + '/' + self.model_name
        return model_path


def exec_trainning():
    train.set_hyperparameters()
    train.trainning()
    train.evaluate()
    train.save_model_and_get_path()



model_design = Model()
train = Train(model_design, 'simple_convnet.h5')
exec_trainning()
